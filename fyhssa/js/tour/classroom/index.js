...
function clearAll() {
	...
	document.getElementById('xinzheng').style.display = "none";
	document.getElementById('siyuan').style.display = "none";
...
function assets() {
	var allassets;
	for(var i=1; i<=9; i++) {
		allassets = allassets + '<img id="' + (i) + '" src="../images/classroom/(' + i + ').webp" rotation="0 -90 0">';
	}
	document.getElementById("assets").innerHTML = allassets;
}

function spheres() {
	var all = '<a-entity class="scene1"><a-sphere id="box1" color="red" onclick="run(1)" position="30 -5.16 0" radius="0.001" material="" geometry=""> </a-sphere></a-entity>';
		var i = 2;
...
function openNav() {
  ...
  function changetitle(a) {
  	if(a <= 10 && a > 2) {
  		document.getElementById('siyuan').style.display = "block";
  	} else {
  		document.getElementById('xinzheng').style.display = "block";
  	}
  }