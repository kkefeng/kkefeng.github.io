...
function clearAll() {
	...
	document.getElementById('rukou').style.display = "none";
...
function assets() {
	var allassets;
	for(var i=1; i<=11; i++) {
		allassets = allassets + '<img id="' + (i) + '" src="../images/parking/(' + i + ').webp" rotation="0 -90 0">';
	}
	document.getElementById("assets").innerHTML = allassets;
}

function spheres() {
	var all = '<a-entity class="scene1"><a-sphere id="box1" color="red" onclick="run(1)" position="0 -5 20" radius="0.001" material="" geometry=""> </a-sphere></a-entity>';
		var i = 2;
...
function addassets() {
 ...
 function changetitle(a) {
  	if(a == 11) {
  		document.getElementById('rukou').style.display = "block";
  	} else if(a < 11) {
