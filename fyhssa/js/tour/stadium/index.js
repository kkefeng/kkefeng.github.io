...
function clearAll() {
	...
	document.querySelector("#xx7").setAttribute('radius', '0.001');
...
function setscene(scene) {
	...
	switch(scene) {
		...
		case 7:
			...
			document.querySelector("#xx" + 7).setAttribute('radius', '1');
			break;
...
function assets() {
	var allassets;
	for(var i=1; i<=7; i++) {
		allassets = allassets + '<img id="' + (i) + '" src="../images/stadium/(' + i + ').webp" rotation="0 -90 0">';
	}
	document.getElementById("assets").innerHTML = allassets;
}

function spheres() {
	var all = '<a-entity class="scene1"><a-sphere id="box1" color="red" onclick="run(1)" position="3 -5 25" radius="0.001" material="" geometry=""> </a-sphere></a-entity>';
		var i = 2;
