...
function clearAll() {
	...
	document.querySelector("#xx3").setAttribute('radius', '0.001');
	document.querySelector("#xxx3").setAttribute('radius', '0.001');
...
function setscene(scene) {
	...
	switch(scene) {
		...
		case 3:
			...
			document.querySelector("#xx" + 3).setAttribute('radius', '1');
			document.querySelector("#xxx" + 3).setAttribute('radius', '1');
			break;
	}
}
...
function assets() {
	var allassets;
	for(var i=1; i<=3; i++) {
		allassets = allassets + '<img id="' + (i) + '" src="../images/ab/(' + i + ').webp" rotation="0 -90 0">';
	}
	document.getElementById("assets").innerHTML = allassets;
}

function spheres() {
	var all = '<a-entity class="scene1"><a-sphere id="box1" color="red" onclick="run(1)" position="30 -7 0.1" radius="0.001" material="" geometry=""> </a-sphere></a-entity>';
		var i = 2;
